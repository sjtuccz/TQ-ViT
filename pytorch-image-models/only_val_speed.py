...
# for token-wise reparameterization
...
def validate(args):
    ...
    print(f"Speed validation for {args.model} with input size ({C}, {H}, {W})")
    print(f"Warm up {warmup} batch...")
    with torch.no_grad():
    ...
    if hasattr(model, 'timer') and model.timer:
        tq_timer = None
        from timm.models.tq_block import TQ_Qscale_deQscale
        for name, module in model.named_modules():
            if isinstance(module, TQ_Qscale_deQscale) and hasattr(module.__class__, 'timer'):
                tq_timer = module.__class__.timer
                break
        model.timer.reset()
        if tq_timer:
            tq_timer.reset()
    ...
    results = OrderedDict(
        ...
        Speed=f"{throughput:.2f} images/sec"
    )
