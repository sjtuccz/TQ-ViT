...
group.add_argument('--teacher-pretrained', action='store_true', default=False,
                   help='Start with pretrained version of specified teacher network (if avail)')
...
group.add_argument('--lr-decay', type=float, default=0.1, metavar='LR',
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if has_no_sync and not need_update:
            ...
            #         print(f"{epoch :}Parameter {name} is not used and has no gradient.")
            _backward(loss)
        ...
        if update_idx % args.log_interval == 0:
            ...
            if args.distributed:
                reduced_loss = utils.reduce_tensor(loss[0].data, args.world_size)
            ...
            if utils.is_primary(args):
                ...
                _logger.info(
                    ...
                    f'LR: {lr:.3e} '
                    f'D: {data_time_m.val:.3f} ({data_time_m.avg:.3f})'
                    f'grad:{total_norm:.2f}'
                )
